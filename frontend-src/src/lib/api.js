...
export const auth = {
  ...
  register:    (name, email, password, referralCode, anonSessionId, phone) => apiFetch(`/api/auth/register`, { method: `POST`, body: JSON.stringify({ name, email, password, ...(phone ? { phone } : {}), ...(referralCode ? { referralCode } : {}), ...(anonSessionId ? { anonSessionId } : {}) }) }),
