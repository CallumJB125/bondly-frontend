...
export default function Login() {
  ...
  async function handleRegister(e) {
    ...
    setLoading(true);
    try {
      const data = await register(name.trim(), email, password, referralCode || undefined, phone.trim() || undefined);
  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        ...
        {redirectReason && !sessionExpired && !magicError && (
          <div style={{ background: 'rgba(108,187,167,0.1)', border: '1.5px solid var(--mint)', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: '0.875rem', color: 'var(--text-primary)' }} role="alert">
            Please sign in to view your <strong>{redirectReason}</strong>.
          </div>
        )}
        ...
        {tab === 'register' && (
          <form onSubmit={handleRegister} className="auth-form" role="tabpanel" id="auth-panel-register" aria-labelledby="auth-tab-register" tabIndex={-1}>
            ...
            <Input label="Phone number" id="regPhone" type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="082 000 0000" autoComplete="tel" />
            <div className="auth-pw-wrap">
        ...
        {tab === 'magic' && (
          ...
          {magicSent ? (
            ...
            <>
              <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: 'var(--space-4)', marginTop: 0 }}>
