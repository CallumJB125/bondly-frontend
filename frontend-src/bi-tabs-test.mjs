...
const tabScreenshots = { 'Overview': '/tmp/bi-overview.png', 'Trends': '/tmp/bi-trends.png', 'Networks': '/tmp/bi-networks.png' };
for (const [label, path] of Object.entries(tabScreenshots)) {
  const tabs = await page.$$('button.bi-tab-btn');
  for (const tab of tabs) {
    const text = await tab.textContent();
    if (text && text.includes(label)) { await tab.click(); break; }
  }
