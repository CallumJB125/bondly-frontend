...
export default function Nav() {
  ...
  // Also hide when the Hook is rendering at /. The Hook has its own minimal top bar.
  if (location.pathname === '/') {
    const params = new URLSearchParams(location.search);
    const forceHook    = params.get('force-hook') === '1';
    const forceLanding = params.get('force-landing') === '1';
    let seen = false;
    try { seen = localStorage.getItem('bondly_hook_seen') === '1'; } catch {}
    if (forceHook || (!forceLanding && !user && !seen)) return null;
  }
    ...
    <>
        ...
        <div className="nav-desktop__inner container">
          ...
          <div className="nav-desktop__links">
            {user ? (
              ...
              <>
                <Link to="/mortgage-readiness" className="nav-path-cta nav-path-cta--buy">Buy a home →</Link>
          ...
          <div className="nav-desktop__actions">
            <button className="nav-theme-btn" onClick={toggleTheme} aria-label={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'}>
              <span aria-hidden="true">{theme === 'light' ? ICON.moon : ICON.sun}</span>
            </button>
            {user ? (
              <>
              ...
              <>
                <a href="https://wa.me/27796971786" target="_blank" rel="noopener noreferrer" className="nav-phone-link" aria-label="WhatsApp us: +27 79 697 1786">+27 79 697 1786</a>
                <Link to="/login" className="btn btn--ghost btn--sm">Sign in</Link>
                <Link to="/mortgage-readiness" className="btn btn--lime btn--sm">Get started →</Link>
              </>
            )}
          </div>
        </div>
      ...
      {user && (
        <div className="nav-mobile-topbar">
          ...
          <div className="nav-mobile-topbar__actions">
            <button className="nav-mobile-topbar__qualify" onClick={() => navigate('/mortgage-readiness')}>
              Check my bond
            </button>
            <button
